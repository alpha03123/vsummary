...
CASE_PRESETS: dict[str, tuple[str, str]] = {
    "series-summary": (
        "series|1|series-home|speed-profile-series-summary",
        "这个系列主要讲了哪些主题？",
    ),
    "series-locate": (
        "series|1|series-home|speed-profile-series-locate",
        "这个系列里哪里讲过 Nacos 3？尽量指出视频和大致位置",
    ),
    "video-summary": (
        "video|1|1-4 准备工作：百度地图API秘钥(AK)|overview|speed-profile-video-summary",
        "这个视频主要讲了什么？",
    ),
    "save-note": (
        "video|1|1-4 准备工作：百度地图API秘钥(AK)|notes|speed-profile-save-note",
        "帮我记一下这个视频的重点",
    ),
}
...
def _attach_service_profiling(stack: ExitStack, service, profiler: Profiler) -> None:
    ...
    input_builder = getattr(service, "input_builder", None) or getattr(service, "_input_builder", None)
    context_loader = getattr(input_builder, "context_loader", None)
    if context_loader is not None:
        stack.enter_context(_wrap_method(context_loader, "load", profiler, "context_loader.load"))

    session_recorder = getattr(service, "session_recorder", None) or getattr(service, "_session_recorder", None)
    session_store = getattr(session_recorder, "session_store", None)
