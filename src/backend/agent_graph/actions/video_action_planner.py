...
from backend.agent_graph.prompts import VIDEO_ACTION_PLANNER_SYSTEM_PROMPT, build_note_length_instruction
...
def _render_tool_instructions(note_length: str) -> str:
    ...
    for tool_name in VIDEO_ACTION_TOOL_MODELS:
        ...
        if tool.name.value == "save_note":
            description = f"{description}{build_note_length_instruction(note_length)}"
