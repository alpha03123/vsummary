...
class PlannedVideoToolCall(BaseModel):
    ...
    Attributes:
        tool_name: 目标工具名，仅允许 `open_notes` / `video_seek`。
    ...
    tool_name: Literal["open_notes", "video_seek"]
...
class VideoActionPlanner:
    ...
    关键不变量：
        - 只暴露 `open_notes` / `video_seek` 两类工具，
...
def _build_tool_call_payload(item: PlannedVideoToolCall) -> dict[str, object]:
    ...
    `open_notes` 只需名称；`video_seek` 携带跳转相关字段。
    """
...
def _render_tool_instructions() -> str:
