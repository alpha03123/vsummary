...
def load_settings(config_path: Path, root_dir: Path) -> AppSettings:
    ...
    workspace_ui_settings = WorkspaceUiSettings(
        ...
        series_markdown_export_path=_normalize_string(
            workspace_ui_payload.get("series_markdown_export_path"),
            default="",
        ),
    )
...
def _render_settings_toml(settings: AppSettings) -> str:
    ...
    lines = [
        ...
        f"series_markdown_export_path = {_toml_string(settings.workspace_ui.series_markdown_export_path)}",
        "",
        "[debug]",
