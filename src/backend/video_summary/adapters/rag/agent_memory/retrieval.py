...
from backend.video_summary.adapters.rag.agent_memory.fastembed_adapter import build_fastembed_embedding
from backend.video_summary.adapters.rag.agent_memory.filters import build_filters
from backend.video_summary.adapters.rag.agent_memory import lancedb_store
from backend.video_summary.adapters.rag.agent_memory.lancedb_store import (
...
class SeriesRetrievalService:
    ...
    def refresh_series(self, series_id: str) -> None:
        with self._index_lock:
            if not lancedb_store.table_exists(self._db_uri, INDEX_TABLE_NAME):
    ...
    def delete_series(self, series_id: str) -> None:
        with self._index_lock:
            ...
            self._finalize_incremental_mutation(series_id)
            try:
                lancedb_store.optimize_lancedb_table(self._db_uri, INDEX_TABLE_NAME)
            except Exception:
    ...
    def _get_or_build_index(self) -> VectorStoreIndex:
        ...
        if loaded is not None:
            self._index = loaded
            self._series_signatures = lancedb_store.read_signature_file(self._db_uri, INDEX_TABLE_NAME) or {}
            return loaded
    ...
    def _rebuild_index(self) -> VectorStoreIndex:
        ...
        lancedb_store.write_signature_file(self._db_uri, INDEX_TABLE_NAME, signatures)
        return self._index
    ...
    def _is_incremental_mutation_ready(self) -> bool:
        return (
            lancedb_store.table_exists(self._db_uri, INDEX_TABLE_NAME)
            and lancedb_store.read_signature_file(self._db_uri, INDEX_TABLE_NAME) is not None
        )

    def _finalize_incremental_mutation(self, series_id: str) -> None:
        self.invalidate()
        signatures = lancedb_store.read_signature_file(self._db_uri, INDEX_TABLE_NAME) or {}
    ...
    def _is_series_signature_stale(self, series_id: str) -> bool:
        ...
        if stored_signatures is None:
            stored_signatures = lancedb_store.read_signature_file(self._db_uri, INDEX_TABLE_NAME) or {}
    ...
    def _delete_video_rows(self, *, series_id: str, video_id: str) -> None:
        lancedb_store.delete_rows(
            ...
            table_name=INDEX_TABLE_NAME,
            where=(
                f"metadata.series_id = '{lancedb_store.escape_lance_string(series_id)}' "
                f"and metadata.video_id = '{lancedb_store.escape_lance_string(video_id)}'"
            ),
        )
    ...
    def _delete_series_rows(self, *, series_id: str) -> None:
        lancedb_store.delete_rows(
            db_uri=self._db_uri,
            table_name=INDEX_TABLE_NAME,
            where=f"metadata.series_id = '{lancedb_store.escape_lance_string(series_id)}'",
        )
