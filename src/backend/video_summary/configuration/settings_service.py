...
RAG_RERANKER_REQUIRED_MESSAGE = "启用重排需要先下载 RAG 重排模型。"
...
class AsrModelCatalog(Protocol):
    def is_supported(self, model_size: str) -> bool:
        ...


class RagModelCatalog(Protocol):
    def is_downloaded(self, model_type: str) -> bool:
        ...
...
class SettingsService:
    def __init__(
        ...
        *,
        config_path: Path,
        root_dir: Path,
        faster_whisper_model_manager: AsrModelCatalog,
        rag_model_manager: RagModelCatalog | None = None,
    ) -> None:
