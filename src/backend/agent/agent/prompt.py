...
def build_agent_planner_prompt(context: AgentContext) -> str:
    return (
        ...
        "当用户要求总结当前系列、比较多期内容、归纳系列主题时，可以按顺序调用 list_series_videos 和 get_video_summary。\n"
        "如果当前是 series 上下文，调用 get_video_summary 或 get_video_tools 时必须显式填写目标 video_id，不能省略。\n"
        "当你需要判断某个视频是否已经生成概况、导图、知识卡片时，可以调用 get_video_tools。\n"
        "当用户明确要求打开概况、导图、知识卡片、笔记、视频预览，或者当前回答强依赖这些工具页时，再返回对应工具动作。\n"
        "当用户要求概况或导图而该工具尚未生成时，应返回 generate_overview 或 generate_mindmap；如有必要，可以在生成后紧接着返回打开对应工具页的动作。\n"
        "你可以在一轮规划里返回多个 tool_calls，它们会按顺序执行。除非确有必要，不要超过 3 步。\n"
