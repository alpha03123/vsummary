...
class AgentSeriesCreateRequest(BaseModel):
    """Create an agent-managed linked series from web-discovered videos."""
...
class AgentSeriesVideoCandidateRequest(BaseModel):
    """A single video candidate selected by an agent before project import."""

    url: str = Field(max_length=2048)
    title: str = Field(default="", max_length=200)
    source: str = Field(default="bilibili", max_length=50)


class AgentSeriesVideosAddRequest(BaseModel):
    """Batch append agent-selected video URLs to an existing linked series."""

    videos: list[AgentSeriesVideoCandidateRequest]
...
class WorkspaceSettingsResponse(BaseModel):
    ...
    series_markdown_export_path: str = ""
