...
class GenerateVideoSummaryTests(unittest.TestCase):
    def test_run_writes_cleaned_transcript_and_returns_summary(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn(("probe", 5.0, "正在探测视频时长"), progress_reporter.events)
            self.assertIn(("extract_audio", 15.0, "正在提取音频"), progress_reporter.events)
            self.assertIn(("transcribe", 42.0, "Whisper 正在转写"), progress_reporter.events)
            self.assertIn(("enhance_transcript", 78.0, "正在纠正转写文本"), progress_reporter.events)
