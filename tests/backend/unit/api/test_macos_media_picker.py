...
class MacMediaPickerTests(unittest.TestCase):
    def test_returns_multiple_unicode_paths_without_loading_tk(self):
        ...
        with patch("backend.api.local_media_picker.sys.platform", "darwin"), patch(
            "backend.api.local_media_picker.subprocess.run",
    ...
    def test_cancel_is_an_empty_selection(self):
        with patch("backend.api.local_media_picker.sys.platform", "darwin"), patch(
            "backend.api.local_media_picker.subprocess.run", return_value=CompletedProcess([], 0, "\n", ""),
        ):
            self.assertEqual(select_local_media_paths(allow_multiple=False), [])

    def test_dialog_failure_is_reported(self):
        with patch("backend.api.local_media_picker.sys.platform", "darwin"), patch(
            "backend.api.local_media_picker.subprocess.run", return_value=CompletedProcess([], 1, "", "failure"),
        ):
