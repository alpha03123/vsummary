...
from backend.video_summary.adapters.media.faster_whisper_transcriber import (
...
class FasterWhisperTranscriberTests(unittest.TestCase):
    def test_cpu_transcriber_does_not_require_nvidia_packages(self) -> None:
        ...
        try:
            with patch(
                "backend.video_summary.adapters.media.faster_whisper_transcriber._ensure_windows_cuda_dll_dirs",
    ...
    def test_missing_nvidia_namespace_is_ignored_when_discovering_cuda_dlls(self) -> None:
        with patch(
            "backend.video_summary.adapters.media.faster_whisper_transcriber.importlib.util.find_spec",
