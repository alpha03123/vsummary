...
- `videos/`、`workspace/`：旧版本的原始视频和工作产物；迁移后不会自动删除
- `data/models/`：本地模型文件

导入本地视频可选择复制、硬链接或外部路径引用（界面称“软链接”）。硬链接要求源文件与 Blob 存储位于同一分区；外部路径引用不会复制文件，但移动源文件后需要重新关联。旧系列迁移会保留原有存储方式；旧硬链接系列若与 Blob 存储跨卷，迁移会报错，不会悄悄改成复制。删除系列会直接删除应用侧的视频 Blob 文件，不经过回收站；硬链接和外部路径引用的原始文件仍会保留。
